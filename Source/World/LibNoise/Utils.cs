...
namespace Game.LibNoise
{
    class Utils
    {
	    ...
	    public static double[] LatLonToXYZ(double latitude, double longitude) {
		    double r = Math.Cos(Utils.toRadians(latitude));
		    double x = r * Math.Cos(Utils.toRadians(longitude));
		    double y = Math.Sin(Utils.toRadians(latitude));
		    double z = r * Math.Sin(Utils.toRadians(longitude));
        ...
        public static double toRadians(double angle) { return Math.PI * angle / 180.0; }
